...
(* 如果 x 的类型为 X 而 y 的类型为 Y， 那么 (x,y) 的类型就是 X×Y。 *)
...
(** **** Exercise: 2 stars, standard (filter_even_gt7) 
    ...
    7. *)
Definition filter_even_gt8 (l:list nat):list nat
...
Example test_map4:map (fun n=>[evenb n;oddb n]) [2;1;2;5]
...
Proof. intros X Y f l. induction l as [|h t IHt].
  - reflexivity.
  - simpl. rewrite map_distr. rewrite IHt. simpl. reflexivity.
  Qed.    
...
Check empty.
Check br 3 (br 2 empty empty) empty.
Check br true empty (br false empty empty). 
...
Proof. (* FILL IN HERE *) 
  unfold zero. unfold succ. unfold two. reflexivity.
Qed.
