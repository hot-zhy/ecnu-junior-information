...
Print conj_fact.
Definition conj_fact' : forall P Q R, P /\ Q -> Q /\ R -> P /\ R.
  intros P Q R H1 H2. destruct H1 as [HP _]; destruct H2 as [_ HR].
  split. apply HP. apply HR. Defined.
...
Proof.
(* FILL IN HERE *) 
  intros X x y H P Hx. destruct H as [x].
  apply Hx.
Qed.
(** [] *)
...
Proof.
(* FILL IN HERE *) 
  intros X x y H. generalize (H (fun z => z = x)).
  intro H1. assert (H2 : x = x). { reflexivity. }
  apply H1 in H2. rewrite H2. apply eq_refl.
Qed.
