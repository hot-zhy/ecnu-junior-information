...
(** To use the [apply] tactic, the (conclusion of the) fact
    ...
    swapped. *)
    (* 不过我们可以用 symmetry 策略 它会交换证明目标中等式的左右两边。 *)
...
Proof.
  (* FILL IN HERE *) 
  intros X x y z l j e1 e2. rewrite e2 in e1.
...
Proof.
  ...
  - simpl. intros m eq. destruct m as [|m'].
    + discriminate eq.
    + simpl in eq. injection eq as goal. apply f_equal. apply IHn'. apply goal.
Qed.        
...
Proof.
  ...
  - intros m e. destruct m as [| m'].
    ...
    + apply f_equal. apply IH. 
      ...
      rewrite <- plus_n_Sm in e. simpl in e. injection e as goal. apply goal.
Qed.
(** [] *)
...
Proof.
  ...
  - intros n eq. destruct n as [| n'].
    + discriminate eq.
    + simpl. apply IHt. injection eq as goal. apply goal.
Qed.
...
Proof.
  ...
  - intros l1 l2 e. simpl in e. 
    injection e as e1 e2. rewrite <- e1; rewrite <- e2. reflexivity. 
  - intros l1 l2 e.  simpl in e. destruct (split t) as [t1 t2].
    injection e as e1 e2. rewrite <- e1; rewrite <- e2. simpl. apply f_equal. 
...
Proof.
  (* FILL IN HERE *) 
  Search eqb.
  intros n m. destruct (eqb n m) eqn : Hnm.
  - apply eqb_true in Hnm. rewrite Hnm. apply eqb_refl.
  - destruct (eqb m n) eqn : Hmn.
    + apply eqb_true in Hmn. rewrite Hmn in Hnm.
      rewrite <- eqb_refl in Hnm. discriminate Hnm.
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl in eq. destruct (test h) eqn : TF.
    + injection eq as e1. rewrite <- e1.  apply TF.
    + apply IHt. apply eq.
Qed.
(** [] *)
...
Proof. (* FILL IN HERE *) 
  intros X test l. induction l as [| h t IHht].
  - reflexivity. 
