...
Proof.
  intros A B HA HB. split. apply HA. apply HB. Qed.   
...
Proof.
  intros m n H.
  destruct H as [Hm Hn].
  rewrite Hm. rewrite Hn. reflexivity.  
Qed.
...
Theorem not_true_is_false' : forall b : bool,
  b <> true -> b = false.
Proof.
  intros b H. destruct b eqn:HE.          (* note implicit [destruct b] here *)
  - (* b = true *)
    unfold not in H.
    exfalso.                (* <=== *)
    apply H. reflexivity.
...
(** **** Exercise: 3 stars, standard, especially useful (All) 
    ...
    (* 以 In 作为参考，请写出递归函数 All，它陈述某个 P 对列表 l 中的所有元素成立。为了确定你的定义是正确的，请在下方证明 All_In 引理。 *)
...
(* 直接将我们想要实例化的参数应用到plus_comm上，就像将一个多态函数应用到参数类型上那样 *)
...
Proof.
  ...
  - intro H. destruct (eqb x y) eqn: E.
    + rewrite eqb_eq in E. rewrite E in H. exfalso.
      apply H. reflexivity. 
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  (* FILL IN HERE *) 
  apply H. right. intro HP. apply H. left. apply HP.
Qed.
(** [] *)
