...
Check ev 0.
Check ev.
(* 在 Coq 中，我们可以认为 ev 定义了一个性质 ev : nat → Prop
，其包括 “证据构造子” ev_0 : ev 0 和 ev_SS : ∀ n, ev n → ev (S (S n))。 *)
...
Proof.
  intros. destruct H as [|n' E'] eqn:EE.
  - left. reflexivity.
...
Theorem evSS_ev_remember : forall n,
  ev (S (S n)) -> ev n.
Proof. 
  intros. remember (S (S n)) as k eqn:Hk. destruct H as [|n' E'] eqn:EE'.
  - discriminate Hk.
...
Proof.
  ...
  destruct H as [H0|H1].
  - discriminate H0.
  - destruct H1. destruct H as [H2 H3]. injection H2 as H4.
  rewrite H4. apply H3.
Qed.
...
Theorem inversion_ex1 : forall (n m o : nat),
  ...
  [n] = [m].
Proof.
  (* inversion可以替代injection和rewrite *)
  (* intros n m o H. injection H as H1 H2. rewrite H1. rewrite H2.  *)
...
Proof.
  ...
  - unfold even in IH. destruct IH.
  unfold even.
  rewrite H. exists (S x). simpl.
  reflexivity.
Qed.
...
Proof.
  intros.
  induction H0 as [|n' En' IHn'].
  - simpl in H. apply H.
  - simpl in H. apply evSS_ev in H. apply IHn' in H. apply H.
Qed.
...
Inductive lt' : nat -> nat -> Prop :=
  | lt_n (n : nat)                : lt' n (S n)
  | lt_S (n m : nat) (H : lt' n m) : lt' n (S m).

Example test_lt': lt' 3 7.
Proof. apply lt_S. apply lt_S. apply lt_S. apply lt_n.
Qed.
...
Inductive total : nat -> nat -> Prop :=
 | to (n m : nat) : total n m.


Example test_to1 : total 101 39.
Proof. apply to. Qed.

Inductive empty_relation : nat -> nat -> Prop :=
.

Example test_empty : ~ (empty_relation 3 7).
...
Example test_er2 : ~ empty_relation 2 7.
Proof. intro H. inversion H. apply H0. Qed.
...
(* Modefied on 22/11/2021*)
Inductive COdd : nat -> nat-> Prop :=
  | CO1 : COdd 1 3
  | CO2 m n (H : COdd m n) : COdd (S (S m)) (S (S n)).

Example test_COdd1 : COdd 3 5.
Proof. apply CO2. apply CO1. Qed.

Print test_COdd1.

Print test_COdd1.

Inductive Rsum : nat -> nat -> nat -> Prop :=
 | Rs1 (n : nat) : Rsum 0 n n
 | Rs2 (n m o : nat) (H : Rsum n m o)
       : Rsum (S n) m (S o).

Example test_Rsum : Rsum 3 5 8.
Proof. apply (Rs2 _ _ _ (Rs2 _ _ _ (Rs2 _ _ _ (Rs1 5)))).
Qed.




Inductive Rminus : nat -> nat -> nat -> Prop :=
  | R1 n : Rminus n 0 n
  | R2 m n o (H : Rminus m n o) : Rminus (S m) (S n) o.

Example test_Rminus : Rminus 5 3 2.
Proof. apply (R2 _ _ _ (R2 _ _ _ (R2 _ _ _ (R1 _ )))).
Qed.

Print test_Rminus.

Inductive Rminus' : nat -> nat -> nat -> Prop :=
  | R1' : forall n, Rminus' n 0 n
  | R2' : forall m n o,
    Rminus' m n o -> Rminus' (S m) (S n) o.
...
intros n l H. induction l as [| h t IHt].
  ...
  - simpl in H. apply plus_eq0 in H. destruct H as [H1 H2].
    apply IHt in H2. destruct (n =? h) eqn : E.
    + inversion H1.
    + intro H4. simpl in H4. destruct H4 as [H4 | H4].
      * rewrite H4 in E. rewrite <- eqb_refl in E. inversion E.
