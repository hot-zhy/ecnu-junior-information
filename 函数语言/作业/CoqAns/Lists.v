...
(* Exercise 10/25/2020 *)
(* 1. Define the function (total l) to get the total sum of all the numbers in bag l. *)
...
(* 2. Define the function (max l) to get the maximum element in the 
bag s of nonnegative numbers.
*)
Fixpoint max(s:bag):nat:=
  match s with
  |nil=>0
  |h::t=>if (max t)<? h then h
        else max t
  end.
(* 
Fixpoint max (s : bag) : nat :=
...
Example test_max: max [1; 6; 3; 5; 7; 9; 2] = 9.
Proof. reflexivity. Qed.
...
Proof.
  intros l. induction l as [|n l' IHl'].
  - reflexivity.
  - simpl. rewrite app_length. rewrite IHl'. simpl. rewrite <- plus_n_Sm. rewrite<-plus_n_O. reflexivity. 
Qed.
...
Proof.
  (* FILL IN HERE *) 
 intros l1 l2 l3 l4. induction l1 as [| h t IHt].
...
Proof.
 ...
 - reflexivity. 
 - simpl. rewrite eqb_ref. rewrite <- IHt. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros l1 l2 e.
  assert (rev (rev l1)=rev (rev l2)) as H.
...
Proof.
 (* FILL IN HERE *) 
 intros d x y o e. simpl. rewrite e. reflexivity.
Qed.
(** [] *)
End PartialMap.
